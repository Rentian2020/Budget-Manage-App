	...
	(table) => {
		return {
			...
			emailChangeTokenCurrentIdx: uniqueIndex(
				...
				.where(
					sql`((email_change_token_current)::text !~ '^[0-9 ]*$'::text)`,
				),
			emailChangeTokenNewIdx: uniqueIndex("email_change_token_new_idx")
				...
				.where(
					sql`((email_change_token_new)::text !~ '^[0-9 ]*$'::text)`,
				),
			reauthenticationTokenIdx: uniqueIndex("reauthentication_token_idx")
				...
				.where(
					sql`((reauthentication_token)::text !~ '^[0-9 ]*$'::text)`,
				),
...
export const ssoDomainsInAuth = auth.table(
	...
	(table) => {
		return {
			...
			domainNotEmpty: check(
				"domain not empty",
				sql`char_length(domain) > 0`,
			),
		};
	},
);
...
export const oneTimeTokensInAuth = auth.table(
	...
	(table) => {
		return {
			relatesToHashIdx: index(
				"one_time_tokens_relates_to_hash_idx",
			).using("hash", table.relatesTo.asc().nullsLast().op("text_ops")),
			tokenHashHashIdx: index(
				"one_time_tokens_token_hash_hash_idx",
			).using("hash", table.tokenHash.asc().nullsLast().op("text_ops")),
...
export const mfaChallengesInAuth = auth.table(
	...
	(table) => {
		return {
			mfaChallengeCreatedAtIdx: index(
